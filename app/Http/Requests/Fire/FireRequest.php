...
class FireRequest extends APIFormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'contributions.*.increaseFrequency' => 'required_unless:contributions.*.frequency,One-Off|string|in:Never,Monthly,Quarterly,Yearly|exclude_if:contributions.*.frequency,One-Off',
            'contributions.*.increaseAmount'    => [new RequiresIncreaseAmountRule(), 'exclude_if:contributions.*.frequency,One-Off', 'exclude_if:contributions.*.increaseFrequency,Never'],
            ...
            'withdrawals.*.increaseFrequency' => 'required_unless:withdrawals.*.frequency,One-Off|string|in:Never,Monthly,Quarterly,Yearly|exclude_if:withdrawals.*.frequency,One-Off',
            'withdrawals.*.increaseAmount'    => [new RequiresIncreaseAmountRule(), 'exclude_if:withdrawals.*.frequency,One-Off', 'exclude_if:withdrawals.*.increaseFrequency,Never']
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'contributions.*.increaseFrequency' => 'contributions.:index.increaseFrequency is required if contributions.:index.frequency isn\'t One-Off and must be one of: Never, Monthly, Quarterly, Yearly',

            'withdrawals.*.frequency' => 'frequency is required and must be one of: One-Off, Monthly, Quarterly or Yearly',
            'withdrawals.*.increaseFrequency' => 'withdrawals.:index.increaseFrequency is required if withdrawals.:index.frequency isn\'t One-Off and must be one of: Never, Monthly, Quarterly, Yearly'
        ];
    }
}